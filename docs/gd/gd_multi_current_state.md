...
- [x] STEP-GD-22: **room timeout / abandon cleanup（production hardening・実DB QA 済み）**。放置 room/queue/member/message が本番で残り続けないよう定期 cleanup。
      **finished room・result 済み room・現在 waiting の match_queue は絶対に消さない**。random_match/public_lobby/invite は状態・TTL・結果有無だけで判定。
      ...
      - **QA**: cleanup ロジック **PGlite 30/30**・**実DB 32/32**（QA データ backdate：abandoned 削除＋cascade＋queue／recent public・invite 誤削除なし／result・finished 保護／
