...
export function buildFeedbackUser(input: {
  ...
  return [
    ...
    '',
    '出力を短く保つため、詳細（improvements / nextPracticeTasks / crossFeatureHints）は本人だけ埋め、',
    `本人（id="${input.selfParticipantId}"・役割 ${GD_ROLE_LABELS[input.selfRole]}）以外の参加者は`,
    'axisScores・companyImpression（1文）・behaviorTraits のみにしてください（残りは空配列/空オブジェクトで可）。',
    '本人については self ブロック（他機能連携用）も作成してください。文章は簡潔にし、冗長な繰り返しを避けます。',
    '',
    '出力は次の JSON オブジェクトのみ（前後に説明文やコードブロック記号を付けない）:',
    '{',
    '  "feedbacks": [',
    '    { // 本人（id="' + input.selfParticipantId + '"）はフル項目',
    '      "participantId": string,',
    '      "axisScores": { "logic": number, "cooperation": number, "volume": number, "roleExecution": number, "drive": number, "listening": number },',
    '      "companyImpression": string,        // 企業選考での評価のされやすさ（本人2〜3文 / 他は1文・断定しない）',
    '      "behaviorTraits": string[],         // 上の選択肢キーから1〜2個',
    '      "improvements": string[],           // 本人のみ2〜3個（他は[]）',
    '      "nextPracticeTasks": string[],      // 本人のみ2個（他は[]）',
    '      "crossFeatureHints": { "matching": string, "interview": string, "es": string, "selfAnalysis": string } // 本人のみ（他は{}）',
    '    }',
    '    // ... 参加者全員分（本人以外は improvements=[], nextPracticeTasks=[], crossFeatureHints={}）',
    '  ],',
    '  "self": {',
    ...
    '  },',
    '  "overallSummary": string                // GD全体の総括（本人視点。ソロなのでAI参加者との比較も含める・3〜4文）',
    '}',
  ].join('\n');
}
