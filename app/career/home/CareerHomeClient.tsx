...
// ── /career/home の client 本体 ───────────────────────────────────
// 認証・契約の route guard は server 側の app/career/home/page.tsx が持つ。
// ここは「契約済み member であること」が確定した後の描画だけを担当する。
//
// ── 機能カードの定義 ──────────────────────────────────────────────
...
export default function CareerHomeClient() {
  ...
  // 基本情報が未完了なら就活版 /career/profile へ遷移させる side-effect。
  // 受験版は /input/basic だったが、就活版は /career/profile に置き換える。
  //
  // 完了判定は lib/careerRouting/destination.ts の isCareerBasicInfoComplete（純関数）に
  // 委譲する。決済直後の着地（/career/billing/success）や server guard と同じ条件を使い、
  // 「Home では完了扱い / success では未完了扱い」のような割れを起こさないため。
  const basicInfoComplete = isCareerBasicInfoComplete(basicInfo);
  useEffect(() => {
    if (isMounted && !basicInfoComplete) {
      router.replace('/career/profile');
    }
  }, [isMounted, basicInfoComplete, router]);

  if (!isMounted) return null;
  if (!basicInfo || !basicInfoComplete) return null; // mount 済 + 未完了。上記 effect で /career/profile へ replace 中
